...
class placeGNN(nn.Module):
    ...
    def forward(
        ...
        fathers, sons = netlist.graph.edges(etype='points-to')
        hidden_cell_pair_feat = torch.cat([
            hidden_cell_feat[fathers, :],
            hidden_cell_feat[sons, :]
        ], dim=-1)
        edge_dis = torch.nn.functional.softplus(self.edge_dis_readout(hidden_cell_pair_feat)).view(-1)#torch.nn.functional.softplus
...
class HeteroGraphConvLayers(nn.Module):
    ...
    def forward(
        ...
        for heterconvlayer in self.heterographconvlayers:
                ...
                h = heterconvlayer.forward(graph.edge_type_subgraph(['pins', 'pinned']), h,mod_kwargs={'pins': {'edge_feats': hidden_pin_feat},'pinned': {'edge_feats': hidden_pin_feat}})#
                hidden_cell_feat, hidden_net_feat = h['cell'], h['net']
